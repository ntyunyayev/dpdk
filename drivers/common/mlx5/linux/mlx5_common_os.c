...
#endif
    int ret;
    printf("inside mlx_os_pd_prepare\n");
    if (cdev->config.pd_handle == MLX5_ARG_UNSET) {
		printf("inside create\n");
        ret = mlx5_os_pd_create(cdev);
    } else {
		printf("inside import\n");
        ret = mlx5_os_pd_import(cdev);
    }
	printf("handle32 : %d\n",((struct ibv_pd *)cdev->pd)->handle);
    printf("handle_name : %s\n",((struct ibv_pd *)cdev->pd)->context->device->name);
    printf("handle_dev_name : %s\n",((struct ibv_pd *)cdev->pd)->context->device->dev_name);
    printf("handle_dev_path : %s\n",((struct ibv_pd *)cdev->pd)->context->device->dev_path);
    printf("handle_ibdev_path : %s\n",((struct ibv_pd *)cdev->pd)->context->device->ibdev_path);
    printf("handle_cmd_fd : %d\n",((struct ibv_pd *)cdev->pd)->context->cmd_fd);
    printf("handle_async_cmd_fd : %d\n",((struct ibv_pd *)cdev->pd)->context->async_fd);
    if (ret) {
        rte_errno = -ret;
        return ret;
    }
...
int mlx5_get_device_guid(const struct rte_pci_addr *dev, uint8_t *guid, size_t len) {
    char tmp[512];
 ...
 *
 */
void mlx5_os_interrupt_handler_destroy(struct rte_intr_handle *intr_handle,
                                       rte_intr_callback_fn cb, void *cb_arg) {
